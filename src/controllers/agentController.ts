...
import { AgentModelConfig } from '../services/agentModelConfig';
...
export const handleAgentChat = async (req: Request, res: Response) => {
  try {
    ...
    if (!params.model || params.model === 'auto') {
      // Use AgentModelConfig if category matches an agent type, otherwise fall back to MawlanaRouter
      if (category && AgentModelConfig.isAgentType(category)) {
        const agentCfg = AgentModelConfig.get(category);
        params.model = agentCfg.model;
        console.log(`[Agent] Model from AgentModelConfig (${category}): ${params.model}`);
      } else {
        const route = await mawlanaRouter.route(messages, category);
        ...
        } // end if (route.needsRag)
      } // end else (MawlanaRouter fallback)
    } // end if (!params.model || params.model === 'auto')
    ...
    if (isStream) {
      ...
      if (reader) {
        // ReadableStream (fetch body) — read chunks and decode
    ...
    // Normalize response from any provider to OpenAI-compatible shape.
    ...
    if (stateDb && convoId) {
      const assistant = normalized.choices?.[0]?.message?.content;
